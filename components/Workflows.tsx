...
export function AuthWorkflow({ notify }: { notify: Notify }) {
  ...
  const next = async () => {
    ...
    if (step === 2) { setLoading(false); return notify(mode === "login" ? "Connexion sécurisée réussie." : "Compte créé. Passez maintenant à la vérification KYC."); }
...
export function SupportWorkflow({ notify }: { notify: Notify }) {
  ...
  const create = async () => {
    setSubmitting(true);
    const response = await fetch("/api/tickets", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customerId: "jean-direl-demo", category, description }) });
...
export function TransactionWorkflow({ notify }: { notify: Notify }) {
  ...
  const confirm = async () => {
    ...
    const response = await fetch(isBill ? "/api/moov-money/bills" : "/api/moov-money/transactions", { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify(isBill ? { provider: phone, reference, amount: Number(amount), confirmationToken: "demo-confirmation" } : { recipient: phone, amount: Number(amount), confirmationToken: "demo-confirmation" }) });
    const payload = await response.json(); setSubmitting(false);
    if (!response.ok) return notify(payload.error || "La transaction n’a pas pu être exécutée.");
