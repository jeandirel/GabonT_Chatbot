...
import { getCustomerProfile } from "./crm";
...
const allowedActions = new Set(["getBalance", "getTransactions", "getCustomerProfile", "createSupportTicket", "getSupportTicket"]);
...
async function executeAction(toolName: string, input: Record<string, unknown>, customerId?: string) {
  ...
  if (!customerId) return { success: false, error: "AUTHENTICATION_REQUIRED", message: "Connectez-vous pour accéder aux données du compte ou créer une réclamation." };
  if (toolName === "getBalance") return getBalance(customerId);
  if (toolName === "getTransactions") return getTransactions(customerId);
  if (toolName === "getCustomerProfile") return getCustomerProfile(customerId);
  if (toolName === "getSupportTicket") return getTicket(String(input.ticketId || ""), customerId);
...
export async function chat(message: string, userId: string, conversationId?: string, customerId?: string) {
  ...
  for (const call of calls) {
    const output = await executeAction(call.toolName!, call.input || {}, customerId);
